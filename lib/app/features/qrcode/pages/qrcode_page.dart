                ...
                return PeriodosPage(
                    ...
                    callBack: (periodo) {
                      ...
                      Modular.to.pushNamed(
                          '/qrcode/qrcodelist/${widget.controller.cursoSelected?.id}/${periodo.id}');
                    });
              }
              return Container();
            }),
...
class _QRCodeWifiGeneratorState extends State<QRCodeWifiGenerator> {
                                                ...
                                                Column(
                                                  children: [
                                                    RadioListTile<SecurityType>(
                                                      title: Text(SecurityType
                                                          .nopass.name),
                                                      value:
                                                          SecurityType.nopass,
                                                      groupValue: securityType,
                                                      onChanged: (SecurityType?
                                                          value) {
                                                        setState(() {
                                                          securityType = value;
                                                        });
  ...
  Future<void> _generateWifiQRCode(String data) async {
    await Modular.get<QrCodeController>().addQrCode(
        qrCodeModel: QrCodeModel.fromMap({
      "content": data,
      "title": title,
      "type": type,
    }));
